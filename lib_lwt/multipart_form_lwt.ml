...
let stream2 ~identify stream content_type =
  let output, push = Lwt_stream.create () in
  let q = Queue.create () in
  let fresh_id =
    let r = ref 0 in fun () -> incr r; !r in
  let tbl = Hashtbl.create 0x10 in
  let emitters header =
    let id = fresh_id () in
    Queue.push (`Id (header, id)) q;
    ...
    if not (Queue.is_empty q) then begin
      ...
      begin match Queue.pop q with
        | `Id (header, id) ->
          ...
          let stream, bounded_emitter = Lwt_stream.create_bounded 10 in
      ...
      | `Done t ->
        let client_id_of_id id =
          let client_id, _, _ = Hashtbl.find tbl id in client_id
        in
        push None;
        Lwt.return_ok (map client_id_of_id t)
      | `Fail _ ->
